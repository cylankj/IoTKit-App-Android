...
import com.cylan.jiafeigou.activity.efamily.zxing.BeepManager;
import com.cylan.jiafeigou.activity.efamily.zxing.CameraManager;
import com.cylan.jiafeigou.activity.efamily.zxing.CaptureActivityHandler;
import com.cylan.jiafeigou.activity.efamily.zxing.DecodeThread;
...
public final class CaptureActivity extends RootActivity implements SurfaceHolder.Callback, View.OnClickListener {
     ...
     * has_bind_ret
     */
    private static final int EBINDCID_IN_BINDING = 8;

    /**
     * custom binding
     */
    private static final int CUSTOM_BIND = 0;
    /**
     * Cover binding
     */
    private static final int COVER_BIND = 1;
    ...
    public void handleDecode(Result rawResult, Bundle bundle) {
        ...
        Log.d("capture", "result-->" + result);
        if (!result.contains("cid=") || !result.contains("mac=")) {
            ToastUtil.showFailToast(this, getString(R.string.EFAMILY_INVALID_DEVICE));
            return;
        }

        Map<String, String> map = Utils.URLRequest(result);
        cid = map.get("cid");
        mac = map.get("mac");
        if (cid.length() != 12 || mac.length() != 17) {
            ToastUtil.showFailToast(this, getString(R.string.EFAMILY_INVALID_DEVICE));
            return;
        }

        if (MyApp.getIsLogin()) {
            httpRequest(CUSTOM_BIND);
        } else {
            ToastUtil.showFailToast(CaptureActivity.this, "(-" + MyApp.getMsgID() + ")" + getString(R.string.GLOBAL_NO_NETWORK));
        }
    }

    private void httpRequest(int type) {
        MsgBindCidReq mMsgBindCidReq = new MsgBindCidReq(cid);
        mMsgBindCidReq.cid = cid;
        mMsgBindCidReq.is_rebind = type;
        mMsgBindCidReq.timezone = TimeZone.getDefault().getID();
        mMsgBindCidReq.alias = getString(R.string.DOOR_MAGNET_NAME);
        mMsgBindCidReq.mac = mac;
        DswLog.i("send MsgBindCidReq msg-->" + mMsgBindCidReq.toString());
        MyApp.wsRequest(mMsgBindCidReq.toBytes());
    }

    private String spiltMac(String mac){
    ...
    private void displayFrameworkBugMessageAndExit() {
        ...
        builder.setMessage("Camera open error");
    ...
    public void handleMsgpackMsg(int msg, MsgpackMsg.MsgHeader msgpackMsg) {
        if (MsgpackMsg.CLIENT_BINDCID_RSP == msgpackMsg.msgId) {
            ...
            if (mRspMsgHeader.ret == Constants.RETOK) {
                ToastUtil.showSuccessToast(this, getString(R.string.ADD_SUCC_1));
                Intent intent = new Intent();
                intent.setFlags(0);
                setResult(RESULT_OK, intent);
                finish();
            } else {
                if (mRspMsgHeader.ret == EBINDCID_IN_BINDING) {
                    dealReBind();
                } else {
