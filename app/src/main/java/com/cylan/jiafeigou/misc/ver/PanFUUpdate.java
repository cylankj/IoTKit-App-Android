...
public class PanFUUpdate extends BaseFUUpdate {

    private List<String> fileNameList;

    public PanFUUpdate(String uuid, List<String> fileNameList) {
        this.uuid = uuid;
    ...
    private String getWiredContent() {
        final int count = ListUtils.getSize(fileNameList);
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < count; i++) {
            builder.append(fileNameList.get(i));
            if (i < count - 1) {
                builder.append(",");
            }
        }
