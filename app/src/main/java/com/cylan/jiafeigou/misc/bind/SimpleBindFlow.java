        ...
        Observable.just(portrait)
                ...
                .subscribe(ret -> {
                    try {
                        ...
                        try {
                            for (int i = 0; i < 2; i++) {
                                Command.getInstance().sendLocalMessage(deviceAddress, UdpConstant.PORT, code.toBytes());
                        ...
                        try {
                            for (int i = 0; i < 3; i++) {
                                Command.getInstance().sendLocalMessage(deviceAddress, UdpConstant.PORT, setServer.toBytes());
                                Command.getInstance().sendLocalMessage(deviceAddress, UdpConstant.PORT, setLanguage.toBytes());
    ...
    public Observable<Boolean> sendWifiInfo(String uuid, String mac, String ssid, String pwd, int type) {
        return Observable.just("send")
                .flatMap(s -> {
                    ...
                    try {
                        // fix #126372 by lxh
                        Command.getInstance().sendLocalMessage(deviceAddress, UdpConstant.PORT, setWifi.toBytes());
                        Command.getInstance().sendLocalMessage(deviceAddress, UdpConstant.PORT, setWifi.toBytes());
