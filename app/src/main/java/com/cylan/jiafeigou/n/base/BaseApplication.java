...
//            startService(new Intent(this, DataSourceService.class));
        ...
        Scheduler schedulers = Schedulers.newThread();
        Log.d("BaseApplication", "BaseApplication: schedulers" + (schedulers == null));
        Scheduler.Worker worker = schedulers.createWorker();
        Log.d("BaseApplication", "BaseApplication: worker" + (worker == null));
        worker.schedule(() -> appComponent.getInitializationManager().initialization());
        PerformanceUtils.startTrace("SmartcallActivity");
    }
