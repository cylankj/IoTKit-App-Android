...
public class SuggestionChatAdapter extends RecyclerView.Adapter<SuggestionChatAdapter.ChatViewHolder> {
    ...
    public ChatViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == 0) {
            //发送的消息布局
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.fragment_suggestchat_item_send, parent, false);
        } else {
            //接收的消息布局
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.fragment_suggestchat_item_receive, parent, false);
        }
