    ...
    private void decideWhichView() {
        if (device != null && JFGRules.isNeedPanoramicView(device.pid)) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                //可能出错,不是对应的index
                GlideUrl url = new CamWarnGlideURL(alarmMsg.version, i, Security.getVId(JFGRules.getTrimPackageName()),
        ...
        switch (view.getId()) {
            case R.id.imgV_big_pic_download:
                if (basePresenter != null)
                    basePresenter.saveImage(new CamWarnGlideURL(alarmMsg.version, currentIndex, Security.getVId(JFGRules.getTrimPackageName()),
                            uuid, device == null ? 0 : device.regionType, device != null && TextUtils.isEmpty(device.vid)));
                break;
            case R.id.imgV_big_pic_share:
                ...
                fragment.setPictureURL(new CamWarnGlideURL(alarmMsg.version, currentIndex, Security.getVId(JFGRules.getTrimPackageName()),
                        uuid, device == null ? 0 : device.regionType, device != null && TextUtils.isEmpty(device.vid)));
            ...
            case R.id.imgV_big_pic_collect:
                ...
                if (basePresenter != null)
                    basePresenter.collect(currentIndex, alarmMsg, new CamWarnGlideURL(alarmMsg.version, currentIndex, Security.getVId(JFGRules.getTrimPackageName()),
                            uuid, device == null ? 0 : device.regionType, device != null && TextUtils.isEmpty(device.vid)));
                break;
        }
    }
