...
import com.cylan.jiafeigou.cache.db.impl.BaseClientDBSyncManager;
...
//                BSToast.showLong(context, "网络不可以用");
            ...
            BaseClientDBSyncManager.getInstance().markSyncNeeded();
            //改变背景或者 处理网络的全局变量
        } else {
            //改变背景或者 处理网络的全局变量
            AppLogger.d("当前网络可用");
            RxBus.getCacheInstance().post(new RxEvent.NetConnectionEvent(true));
            BaseClientDBSyncManager.getInstance().performSyncTask();
