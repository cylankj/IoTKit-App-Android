...
public class JFGNetWorkMonitorReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (!mobNetInfo.isConnected() && !wifiNetInfo.isConnected() && !DataSourceManager.getInstance().isOnline()) {
...
//            BaseDPTaskDispatcher.getInstance().markSyncNeeded();
            //改变背景或者 处理网络的全局变量
        } else if (DataSourceManager.getInstance().isOnline() && (mobNetInfo.isConnected() || wifiNetInfo.isConnected())) {
            //改变背景或者 处理网络的全局变量
            AppLogger.d("当前网络可用");
