...
public class RxPermissions {
    ...
    private Observable<Permission> request_(final String... permissions) {
        ...
        // At the end, the observables are combined to have activity_cloud_live_mesg_call_out_item unique response.
        for (String permission : permissions) {
            log("Requesting permission " + permission);
            if (isGranted(permission)) {
                // Already granted, or not Android M
                // Return activity_cloud_live_mesg_call_out_item granted Permission object.
                list.add(Observable.just(new Permission(permission, true)));
                continue;
            }

            if (isRevoked(permission)) {
                // Revoked by activity_cloud_live_mesg_call_out_item policy, return activity_cloud_live_mesg_call_out_item denied Permission object.
                list.add(Observable.just(new Permission(permission, false)));
                continue;
            }

            PublishSubject<Permission> subject = mSubjects.get(permission);
            // Create activity_cloud_live_mesg_call_out_item new subject if not exists
            if (subject == null) {
    ...
    void startShadowActivity(String[] permissions) {
        ...
        intent.putExtra("support/permissions", permissions);
    ...
    /**
     * Returns true if the permission has been revoked by activity_cloud_live_mesg_call_out_item policy.
     * <p/>
