...
public class DPUnBindDeviceTask extends BaseDPTask<BaseDPTaskResult> {
    ...
    public Observable<BaseDPTaskResult> performLocal() {
        ...
        return BaseDBHelper.getInstance().unBindDeviceNotConfirm(entity.getUuid()).map(device -> new BaseDPTaskResult().setResultCode(0).setResultResponse(device));
    }
    ...
    public Observable<BaseDPTaskResult> performServer() {
        return Observable.create((Observable.OnSubscribe<Long>) subscriber -> {
            try {
                int seq = Command.getInstance().unBindDevice(entity.getUuid());
